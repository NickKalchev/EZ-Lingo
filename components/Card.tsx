...
function Card({ title, id, imageSrc, onClick, disabled, active }: Props) {
    return (
        <div 
            onClick={() => onClick(id)}
            className={cn("h-full border-2 rounded-xl border-b-4 hover:bg-black/5 cursor-pointer active:border-b-2 flex flex-col items-center justify-between p-3 pb-6 min-h-[160px] min-w-[160px]", disabled && "pointer-events-none opacity-50")}
        >
